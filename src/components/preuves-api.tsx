  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-5">
      <h3 className="flex items-center justify-between text-sm font-semibold text-zinc-900">
        ...
        {ajoutees && ajoutees.length > 0 && (
          <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">
            {ajoutees.length} preuve{ajoutees.length > 1 ? "s" : ""} ajoutée
            {ajoutees.length > 1 ? "s" : ""}
          </span>
        )}
      </h3>
      <p className="mt-1 text-xs text-zinc-500">
        ...
        (plateforme OpendataSoft). Aucune preuve n&rsquo;est ajoutée si la source ne
        répond pas ou ne contient pas de correspondance.
      </p>
        ...
        <form action={formAction} className="mt-3">
          ...
          >
            {pending
              ...
              : "Récupérer les preuves (météo, radar, travaux)"}
          </button>
        </form>
      )}
